...
Статус: **PLANNED**. Этот этап следует после условной фиксации Stage 3 и до экономики/снабжения Stage 4. Он не добавляет экономику, строительство или сложную логистику: цель — задействовать все существующие managed-группы и проверить устойчивость более крупного моторизованного состава.
...
- Любая машина проходит capacity preflight для всех живых членов группы. Политика остаётся `ALL_OR_FALLBACK`: нельзя отправить часть группы на машине, оставив остальных пешком.
...
1. Увеличить initial/replacement roster до пяти и обновить AI-limit projection.
2. Проверить пехотный baseline `4 × 5` с прежними ролями, чтобы отделить ошибки размера группы от нового планирования.
3. Включить `3 ATTACK / 1 FORWARD_DEFEND_QRF`, распределение целей и role hysteresis.
4. Расширить vehicle eligibility/cap до четырёх slot на сторону и добавить выбор truck/light transport по вместимости.
5. Выполнить controlled transport test, 30-минутную headless-матрицу и двухчасовой soak до начала Stage 4.
...
5. Каждая eligible живая группа получает не более одной active/reserved машины; active cap не превышает четыре на фракцию.
6. `BOARDING_COMPLETE` возможен только при settled `5/5`; недостаточная вместимость приводит к выбору другого транспорта либо bounded infantry fallback.
