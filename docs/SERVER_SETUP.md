...
Это пока не готовая схема публичного Workshop-сервера. Репозиторий не содержит
`.pak`, Workshop metadata и pipeline публикации. Для подключения клиент должен
иметь тот же checkout исходников и запускаться с теми же addon GUID. Публичный
сервер для произвольных игроков потребует отдельной упаковки и публикации
обоих аддонов.
