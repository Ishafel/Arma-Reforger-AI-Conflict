...
Crew-role и mobility recovery используют независимые budgets, оба ограниченные `aicfVehicleMaxRecoveries`. Settled driver/gunner лишь armed pending evidence в `AICF_VehicleMovementState`, не изменяет route/motion timestamps и не создаёт success. `VEHICLE_CREW_RECOVERY_SUCCEEDED` и общий recovery success разрешены только после последующего требуемого physical motion либо route progress.
...
Перед любым authority delete `AICF_VehicleCleanupManager` сканирует все compartments, managed logical link/get-in/get-out/oriented bounds, target-scoped player transitions и живых игроков не дальше 15 м. Любой blocker сбрасывает stable-clear clock и при длительном ожидании пишет rate-limited `VEHICLE_CLEANUP_DEFERRED`. Удаление разрешено только после непрерывных 5 секунд полного clear и немедленного повторного scan по immutable full identity snapshot. Это консервативный server-side gate; T10/A2 обязаны повторить race посадки игрока.
