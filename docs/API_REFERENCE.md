...
Числа после `ToInt()` ограничиваются безопасными min/max, а ожидаемая сторона принимает только `US` или `USSR`. Для текущих `10 + 10` групп нижняя граница `max_managed_agents` равна 80, стандартное значение — 220. Capacity projection считает READY-группы по фактическому составу, а каждый проверяемый или уже `SPAWNING` slot — по `max(actual_agents, desired_size)`. Глобальный `MAX_GROUP_SIZE=10` не используется как фиксированная плата за pending slot: четырёхместный replacement резервирует четыре места. Проверка выполняется до перехода в `SPAWNING`, поэтому ожидание реального лимита не начинает deployment transaction и не увеличивает `group_generation`. Исторические прогоны с восемью группами и `max_managed_agents=64` остаются валидными только для прежней конфигурации.
...
Источник: [`SCR_WorldTools.c` 1.8.0.10](https://github.com/BohemiaInteractive/Arma-Reforger-Script-Diff/blob/v1.8.0.10/scripts/Game/Global/SCR_WorldTools.c). Штатный Conflict использует тот же terrain-query при создании HQ vehicle в `SCR_CampaignMilitaryBaseComponent`.
...
static bool SCR_WorldTools.TraceCilinderUtil(
    vector pos,
    float radius = 0.5,
    float height = 2,
    TraceFlags flags = TraceFlags.ENTS | TraceFlags.OCEAN,
...
До геометрического query `AICF_VehicleAcquisitionFlow` повторно применяет `AICF_ConflictAdapter.GetSpawnRejectionReason`: base существует и initialized, принадлежит нужной стороне, не contested/capturing, enemies отсутствуют, spawn point enabled/active и имеет правильный faction key. Все safe base сортируются детерминированно по расстоянию и `BaseKey`; затем каждая по очереди получает до 16 точных кандидатов через `FindAllEmptyTerrainPositions`, проверяет поверхность и расстояние всех живых managed members. `VEHICLE_SPAWN_CANDIDATES_EVALUATED` один раз на bounded attempt/редкий wait-probe агрегирует каждую невражескую базу и её точный результат (`CONTESTED`, inactive spawn, `TOO_FAR`, `NO_EMPTY_TERRAIN`, `NO_BOARDING_SITE_WITHIN_RANGE`, `GROUP_NOT_READY` или `SELECTED`); ожидаемые hostile entries представлены только счётчиком `hostile_skipped`.

Непосредственно перед commit сохранённая позиция проверяется именно в своей точке через `TraceCilinderUtil`, а не повторным grid-search вокруг неё. Старый commit вызвал `FindAllEmptyTerrainPositions` с `areaRadius=3` и `cylinderRadius=8`; реализация 1.8 отбрасывает кандидаты за пределом `areaRadius - cylinderRadius`, поэтому при отрицательном допустимом расстоянии результат гарантированно был пустым и все заявки получали ложный `SPAWN_PAD_OCCUPIED`. Каждый exact probe теперь пишет `VEHICLE_SPAWN_PAD_PROBE` с `result`, `stable_slot`, reservation/base, origin, радиусом/высотой цилиндра, фактическими hit-результатами тех же четырёх trace (`entity_id`, `rpl_id`, type, distance, faction) и дополнительным bounded-списком соседних сущностей. `SpawnEntityPrefabEx` вызывается единственный раз и только после выбора полностью допустимого кандидата; если живых бойцов уже нет, возвращается retryable `GROUP_NOT_READY` без entity. Сам spawn выполняется только при `Replication.IsServer()`, `campaign.IsMaster()` и точной Fleet-reserved Lease identity; accepted vehicle generation коммитит только `FactionFleet` при bind.
...
Engine callback `COMPLETED` не является самостоятельным доказательством выполнения стратегического waypoint. Reliability controller сверяет живого лидера с фактической позицией objective; удалённый callback классифицируется как `FALSE_COMPLETION`, не расходует repair budget и создаёт отличающийся navmesh endpoint. После трёх разных endpoint группа получает 15-секундный полевой hold и полный стратегический replan. MOB egress хранит отдельное состояние `EGRESS_BLOCKED_BY_SAFETY`: `PLAYER_FENCE` и `COMBAT_THREAT` приостанавливают hard deadline и acceptance latch, а после исчезновения blocker recovery episode и bounded deadline запускаются заново. При terminal vehicle fallback единственный владелец перехода — `AICF_VehicleTaskHandoff`: `2+` бойцов восстанавливают пеший operational order, один получает retreat waypoint к безопасной союзной MOB, пустая группа остаётся обычному replacement lifecycle.
...
Stage 4 state добавлен в уже реплицируемый `SCR_GameModeCampaign` через `RplProp` + `Replication.BumpMe()`: total/connected supplies, logistics tier, pending reinforcement requests, shipments in transit, bounded summaries десяти slots, faction objective и допустимые order-target labels отдельно для US/USSR. Конкретный stock каждой базы остаётся собственностью vanilla replication. Приказ отправляется через player-owned `SCR_PlayerController` как `slotId + targetCallsign`; authority получает player faction через `SGetPlayerFaction`, применяет rate limit и использует role-specific `IsTargetValidForRole` перед заменой waypoint. Отдельный reliable RPC `slotId + role + unitType + desiredSize` применяет те же faction/bounds/rate-limit gates. Поддерживаются роли `ATTACK/DEFEND/RESERVE`, типы `INFANTRY/MOTORIZED_LIGHT/MOTORIZED_TRUCK/MOTORIZED_ARMED_LIGHT` и размер 1–10; размер применяется к следующему развёртыванию. Armed-light ограничен составом 2–4, тяжёлые профили из командной панели недоступны.

Workbench 1.8 strategic-UI validation `.cache/stage4-strategic-ui-validate-20260815-r6/console.log`: Game `5731 files / 11229 classes`, CRC32 `bf9d53b1`, `Game successfully created`, `SCRIPT(E/F)=0`, `ENGINE(F)=0`; shutdown resource-leak list относится к стандартному Workbench harness.

Последующий critical-fix validate `.cache/stage4-critical-fixes-validate-20260815-r12/console.log`: Game `5731/11231`, CRC32 `6a07dbba`, `Game successfully created`, `SCRIPT(E/F)=0`, `ENGINE(F)=0`, VM `0`.

Direct ServerDiag smoke `.cache/Stage4-CriticalFix-Smoke-R2-20260815-1647/logs/logs_2026-08-15_16-46-42/console.log` работал 211 секунд с `10 + 10` группами. Он зафиксировал четыре полных MOB recovery-эпизода: для каждого `ATTEMPTED=1`, `SUBMITTED=1`, `ACCEPTED=1`, затем отложенное `PHYSICALLY_CONFIRMED=1` по всему живому составу. `PARTIAL=0`, `SUBMISSION_FAILED=0`, `MOB_EGRESS_DEADLINE_MISSED=0`, VM/null `0`, `ORDER_REPAIR_ACCOUNTING_INVARIANT_FAILED=0`; три reliability heartbeat сохранили `accounting_balanced=1` и `unaccounted=0`. Прогон намеренно выполнялся с `aicfVehiclesEnabled=0`, поэтому не является vehicle/armed-light runtime evidence.

Финальный semantic smoke `.cache/Stage4-CriticalFix-Smoke-R3-20260815-1654/logs/logs_2026-08-15_16-54-13/console.log` подтвердил, что обе R0 с приказом `IDLE_RESERVE` получают `allowed_idle_reason=HQ_RESERVE`, `mob_presence_ms=0` и не входят в recovery. Одновременно all-member envelope обнаружил единственного отставшего бойца US A1 (`inside_members=1`), перенёс только его и на следующем аудите подтвердил `alive_members=4 outside_members=4`. `PARTIAL=0`, `MOB_EGRESS_DEADLINE_MISSED=0`, VM/null `0`, accounting invariant failures `0`; reliability heartbeat: `accounting_balanced=1`, `unaccounted=0`.
