...
- Смена роли перестраивает role-local callsign и текущий приказ authoritative commander. Смена типа на motorized запускает отложенный vehicle request, а возврат к infantry завершает активный transport trip. Карточка и карта показывают «Площадка выбрана», «Следует к месту выдачи N/M», «Ожидание бойцов N/M», «Выдача техники» и «Посадка». Фактическая выдача машин требует включённого `aicfVehiclesEnabled=1`; при выключенном Stage 3 группа остаётся пешей.
...
| `aicfVehicleSpawnStagingRadiusMeters` | `12` | радиус, в котором должны находиться все живые бойцы |
...
- `VEHICLE_SPAWN_COMMIT_REQUESTED`, `VEHICLE_SPAWN_COMMIT_REJECTED`, `VEHICLE_SPAWN_PLAN_COMPLETED`, `VEHICLE_SPAWN_PLAN_CANCELLED`.
