...
- примерное прибытие к источнику, получателю или базе приписки при возврате;
- база приписки.
...
Оценка прибытия использует общий с формой снабжения `AICF_LogisticsArrivalEstimate`:
расстояние по прямой × 1,3 / текущая горизонтальная скорость. Результат округляется
вверх до минут; менее минуты отображается отдельно. При скорости ниже 0,5 м/с,
недоступной физике или recovery время уточняется; при погрузке/разгрузке машина
уже у соответствующей базы. После завершения, отмены или смены поколения job
его время не показывается. Число обновляется в существующем серверном снимке
каждые 2 секунды; отдельного клиентского таймера и нового RPC нет.
