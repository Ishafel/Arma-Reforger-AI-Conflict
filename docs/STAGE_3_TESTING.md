...
| `aicfVehicleBoardingTimeoutMs` | `40000` | Soft deadline DRIVER/GUNNER/PASSENGERS, а также deadline высадки и смены экипажа |
| `aicfVehicleBoardingApproachTimeoutMs` | `180000` | Отдельный bounded deadline пешего APPROACH к машине; immutable total cap равен `approach timeout + timeout остальных planned phases`; на всю boarding-попытку возможна ровно одна target-scoped grace 10 секунд с абсолютным hard cap |
