...
Во время рейса область результата расширяется вместо подсказки выбора, чтобы
маршрут, состояние и время помещались над кнопкой «Отправить».
...
автопарк с совместимой грузовой машиной и свободным лимитом техники/AI. За один
...
- Пока запрос ожидает ответа или выполняется, повторная отправка и изменение полей
  отключены. Состояние запроса хранится на player controller, поэтому пересоздание
  виджетов при закрытии карты не теряет его. Выход игрока/смена фракции отменяют
  заявку; reconnect создаёт новый controller, старый рейс не восстанавливается.
...
- `AICF_SupplyTransportRpc` отправляет только намерение через reliable server RPC
  ...
  с текущим status, token и названиями маршрута; Bump вызывается только при изменении.
...
9. Выбрать другую союзную базу назначения, количество в пределах её свободного
   ...
   не должно создавать второй рейс. Закрыть/открыть карту во время подготовки:
   маршрут и актуальный статус сохраняются, повторная отправка отключена.
