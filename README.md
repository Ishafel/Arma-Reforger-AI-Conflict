...
| Stage 3 — наземная техника | **Transport T1–T9 и Armed A1: исторические ручные FAIL; post-T9-патч ожидает T10/A2** | T9 подтвердил per-member `APPROACH`, но выявил разрушительный persistent-stuck recycle полевой группы и недостаточную диагностику выбора spawn-базы; расхождение пассажиров в бою осталось кандидатом. Post-T9-патч сохраняет stuck-группу на достигнутой позиции, логирует исход каждой релевантной базы и владельца AI action при посадке; static audit и Workbench validation пройдены, новый dedicated runtime ещё не запускался |
...
Stage 3 пока является только реализационным кандидатом. Ручные Transport T1–T9 и Armed A1 сохраняют исторический статус `FAIL`; подробные T8/T9-отчёты сохранены в `docs/STAGE_3_TESTING.md`. Post-T9-патч сохраняет persistent-stuck группу и её target на достигнутой позиции, возобновляет операцию после bounded hold или изменения карты, пишет агрегированный результат проверки каждой невражеской spawn-базы и фиксирует current AI action каждого бойца во время посадки. Ранее добавленные bounded `WAITING_FOR_SITE`, functional world pool и 15-метровый/5-секундный delete gate сохранены. Штатные мины Arland не удалялись и не менялись. Текущий snapshot прошёл `tools/Test-Stage3Static.ps1` и Workbench 1.7 `Validate Scripts` по пяти конфигурациям: `.cache/stage3-t9-field-hold-validate-20260809/console.log`, Game CRC32 `9f3c297e`, `Script validation successful`, `SCRIPT (E/F)=0`. Это не runtime-PASS: далее обязательны Transport T10, Armed A2, controlled no-combat boarding repeat, focused timeout/order/cleanup/cap fault-срезы и 30-минутный прогон.
