...
| Stage 3 — наземная техника | **Transport T1–T8 и Armed A1: исторические ручные FAIL; post-T8-патч ожидает T9/A2** | T8 подтвердил happy-path и `SAFE_REUSE`, но выявил зависание общего `APPROACH`, бесконечный spawn-request churn после потери cohesion и опасное удаление исправной abandoned-машины во время посадки игрока. Post-T8-патч заменяет общий waypoint точными per-member actions, вводит bounded `WAITING_FOR_SITE` и безопасный world pool; static audit и Workbench validation пройдены, dedicated runtime ещё не запускался |
...
Stage 3 пока является только реализационным кандидатом. Ручные Transport T1–T8 и Armed A1 сохраняют исторический статус `FAIL`; подробный T8-отчёт сохранён в `docs/STAGE_3_TESTING.md`. Post-T8-патч адресует остановку общего `APPROACH`, зависший spawn-request и cleanup race с игроком через точный per-member staging, bounded `WAITING_FOR_SITE`, `NormalizeAfterVehicle`, функциональный world pool и 15-метровый/5-секундный delete gate. Штатные мины Arland не удалялись и не менялись ни кодом, ни тестовой конфигурацией. Текущий snapshot прошёл `tools/Test-Stage3Static.ps1` и Workbench 1.7 `Validate Scripts` по пяти конфигурациям: `.cache/stage3-t8-fixes-final5-20260809-191200/console.log`, Game CRC32 `013129aa`, `Script validation successful`, `SCRIPT (E/F)=0`. Это не runtime-PASS: dedicated T9/A2 ещё не выполнялись; далее обязательны Transport T9, Armed A2, focused timeout/order/cleanup/cap fault-срезы и 30-минутный прогон.
