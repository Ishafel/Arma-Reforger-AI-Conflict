...
- обнаружение отсутствия прогресса и перестроение маршрута;
...
Для локальной отладки позиций managed AI-групп можно добавить `-aicfDebugMapMarkers 1` одновременно в команды сервера и клиента. На карте появятся восемь глобальных маркеров с цветом стороны, устойчивым слотом, полной ролью, текущей задачей, целью, числом живых бойцов и состоянием route recovery. Режим выключен по умолчанию, раскрывает обе стороны и не предназначен для публичной игры или балансового прогона.
