...
| Stage 3 — наземная техника | **кандидат в разработке** | Ветка `codex/stage-3-ground-vehicles`: транспорт, посадка/высадка, watchdog, пеший fallback и опциональная лёгкая вооружённая техника; ручная runtime-приёмка ещё не выполнялась |
...
- серверное создание техники только у безопасной friendly-базы с проверкой contested/enemy state, пустого участка и дистанции до группы;
- штатная цепочка AI-waypoint `GetIn → Move → GetOut` с назначением водителя, стрелка и пассажиров;
- высадка на настраиваемой дистанции от цели и восстановление прежнего пехотного приказа без изменения правил `CaptureRelay`;
- отдельный vehicle watchdog для boarding timeout, отсутствия водителя/стрелка, отсутствия прогресса, переворота, повреждения и разделения группы;
- ограниченная последовательность восстановления: новый экипаж, перестроение маршрута, высадка, abandoned/cleanup и продолжение пешком;
- атомарный лимит машин на фракцию, поколения группы и машины, защита от двойного spawn и stale runtime;
- состояние `VEH <state>` в диагностическом маркере и отдельный лог-контракт `[AICF][STAGE3]`.

Stage 3 пока является только реализационным кандидатом. Workbench validation и статический lifecycle-аудит не заменяют ручную проверку посадки, движения, потерь экипажа, fallback и лимитов в dedicated runtime.
...
Строка `[AICF][STAGE3][RESULT] ... status=PASS` подтверждает только автоматизируемые инварианты поездки. Финальный Stage 3 PASS требует заполненной ручной матрицы, видео/скриншотов и полных server/client logs.
