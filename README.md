...
Для локальной отладки позиций managed AI-групп можно добавить `-aicfDebugMapMarkers 1` одновременно в команды сервера и клиента. На карте появятся восемь глобальных маркеров с цветом стороны, ролью и стабильным слотом; смена цели не пересоздаёт маркер. Режим выключен по умолчанию, раскрывает обе стороны и не предназначен для публичной игры или балансового прогона; подробности приведены в [`docs/STAGE_1_TESTING.md`](docs/STAGE_1_TESTING.md#опциональные-debug-маркеры-управляемых-ai-групп).
