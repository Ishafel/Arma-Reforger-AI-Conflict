...
$passengerBoarding = 'AIConflictCore/Scripts/Game/AIConflict/Vehicles/AICF_VehiclePassengerBoarding.c'
...
Assert-FileContains $spawner 'SCR_WorldTools\.FindAllEmptyTerrainPositions' 'Vehicle spawning must evaluate multiple empty-terrain positions per safe base'
...
$evaluateBaseSpawnPositionsBody = Get-CMethodBody $spawner '^\s*protected\s+AICF_VehicleSpawnSiteEvaluation\s+EvaluateBaseSpawnPositions\s*\(' 'EvaluateBaseSpawnPositions'
...
Assert-TextSequence $trySpawnBody @('foreach \(SCR_CampaignMilitaryBaseComponent safeCandidate : safeCandidates\)', 'candidateDistanceSq', 'maximumSpawnDistanceMeters', 'siteFailureReason = "TOO_FAR"', 'continue;', 'EvaluateBaseSpawnPositions\(', 'AppendCandidateTrace\(', 'site\.m_sResult == "SELECTED"', 'selectedSite = site', 'spawnBase = safeCandidate', 'break;') 'Every ordered safe base must delegate to multi-position validation before an exact site can be selected'
Assert-TextSequence $evaluateBaseSpawnPositionsBody @('FindAllEmptyTerrainPositions\(', 'm_iCandidatePositionCount <= 0', 'm_sResult = "NO_EMPTY_TERRAIN"', 'for \(int candidatePositionIndex', 'candidatePositions\[candidatePositionIndex\]', '!IsWheeledSpawnSurfaceSuitable\(', 'ReportSurfaceCandidateRejected\(', 'continue;', 'MeasureAliveGroupDistancesToPosition\(', 'aliveCount <= 0', 'm_sResult = "GROUP_NOT_READY"', 'return result;', 'farthestDistanceMeters > maximumBoardingDistanceMeters', 'm_bHasBoardingRejection', 'continue;', 'm_sResult = "SELECTED"', 'm_vPosition = candidatePosition', 'return result;', '!foundValidatedSurface', 'm_sResult = "WATER_OR_UNDRIVABLE_SURFACE"', 'return result;', 'm_sResult = "NO_BOARDING_SITE_WITHIN_RANGE"') 'Every empty-terrain position must pass surface, living-roster and all-member boarding-distance gates before selection'
Assert-TextNotContains $evaluateBaseSpawnPositionsBody 'SpawnEntityPrefabEx' 'Spawn-site evaluation must remain side-effect free and never create a vehicle entity'
Assert-TextSequence $trySpawnBody @('site\.m_sResult == "GROUP_NOT_READY"', 'siteFailureReason = "GROUP_NOT_READY"', 'break;', 'site\.m_sResult == "NO_EMPTY_TERRAIN"', 'siteFailureReason\.IsEmpty\(\) \|\| siteFailureReason == "TOO_FAR"', 'siteFailureReason = "NO_EMPTY_TERRAIN"', 'site\.m_sResult == "WATER_OR_UNDRIVABLE_SURFACE"', 'siteFailureReason = "WATER_OR_UNDRIVABLE_SURFACE"', 'ReportCandidateEvaluation\(', 'if \(boardingRejectionSite\)', 'if \(!spawnBase\)', 'failureReason = siteFailureReason', 'failureBase = siteFailureBase', 'retryable = true', 'return false;') 'Exhausted position sets must preserve the most precise retryable site failure before entity creation'
Assert-TextSequence $trySpawnBody @('site\.m_sResult == "NO_BOARDING_SITE_WITHIN_RANGE"', 'site\.m_bHasBoardingRejection && !boardingRejectionSite', 'boardingRejectionSite = site', 'siteFailureReason = "NO_BOARDING_SITE_WITHIN_RANGE"', 'if \(boardingRejectionSite\)', 'AICF_Stage1Diagnostics\.BaseKey\(boardingRejectionSite\.m_Base\)', 'runtime\.MarkSpawnIssueReported\(reportKey\)', 'm_vBoardingRejectionPosition', 'm_iAliveCount', 'm_fLeaderDistanceMeters', 'm_fNearestDistanceMeters', 'm_fFarthestDistanceMeters', 'm_sMemberSamples', '"VEHICLE_SPAWN_SITE_REJECTED"') 'Only a terminal ordered boarding-site rejection may emit exact telemetry through the runtime one-shot key'
...
Assert-TextSequence $trySpawnBody @('AppendCandidateTrace\(\s*candidateTrace,\s*safeCandidateKey,\s*"TOO_FAR"', 'EvaluateBaseSpawnPositions\(', 'AppendCandidateTrace\(\s*candidateTrace,\s*safeCandidateKey,\s*site\.m_sResult', 'ReportCandidateEvaluation\(', 'if \(boardingRejectionSite\)') 'Every safe base must expose its exact multi-position outcome before the deduplicated nearest-failure event'
Assert-OccurrenceCount $trySpawnBody 'AppendCandidateTrace\(' 3 'TrySpawn must retain exactly unsafe-base, too-far, and evaluated-site aggregate trace points'
...
Assert-FileNotContains $coordinator 'Create(?:Driver|Gunner|Passenger)BoardingWaypoint\s*\(' 'Role-ordered boarding must not call legacy generic group GetIn waypoints'
Assert-FileNotContains $passengerBoarding 'SCR_BoardingEntityWaypoint|CreatePassengerBoardingWaypoint|AddWaypointAt|AddUsableVehicle' 'Exact passenger boarding must not expose a group waypoint or generic vehicle utility'
Assert-FileContains $passengerBoarding 'new\s+SCR_AIGetInVehicle[\s\S]*BaseCompartmentSlot[\s\S]*EAICompartmentType\.Cargo' 'Passenger boarding must use exact per-member Cargo actions'
...
$trackPassengerActionBody = Get-CMethodBody $runtime '^\s*void\s+TrackPassengerBoardingAction\s*\(' 'TrackPassengerBoardingAction'
$passengerStartBody = Get-CMethodBody $passengerBoarding '^\s*bool\s+Start\s*\(' 'PassengerBoarding.Start'
$passengerMaintainBody = Get-CMethodBody $passengerBoarding '^\s*bool\s+Maintain\s*\(' 'PassengerBoarding.Maintain'
$passengerCancelBody = Get-CMethodBody $passengerBoarding '^\s*void\s+Cancel\s*\(' 'PassengerBoarding.Cancel'
$passengerReleaseTrackingBody = Get-CMethodBody $passengerBoarding '^\s*void\s+ReleaseTracking\s*\(' 'PassengerBoarding.ReleaseTracking'
$passengerCancelAllBody = Get-CMethodBody $passengerBoarding '^\s*int\s+CancelAll\s*\(' 'PassengerBoarding.CancelAll'
$passengerClearTrackingBody = Get-CMethodBody $passengerBoarding '^\s*void\s+ClearTracking\s*\(' 'PassengerBoarding.ClearTracking'
$passengerFindCargoBody = Get-CMethodBody $passengerBoarding '^\s*protected\s+bool\s+FindAvailableCargoCompartment\s*\(' 'PassengerBoarding.FindAvailableCargoCompartment'
$passengerDescribeBody = Get-CMethodBody $passengerBoarding '^\s*string\s+Describe\s*\(' 'PassengerBoarding.Describe'
$passengerRollbackBody = Get-CMethodBody $passengerBoarding '^\s*protected\s+void\s+RollbackReservations\s*\(' 'PassengerBoarding.RollbackReservations'
$passengerReleaseReservationBody = Get-CMethodBody $passengerBoarding '^\s*protected\s+void\s+ReleaseReservation\s*\(' 'PassengerBoarding.ReleaseReservation'
$passengerSupportedSettledBody = Get-CMethodBody $passengerBoarding '^\s*protected\s+bool\s+IsSupportedSettledCompartment\s*\(' 'PassengerBoarding.IsSupportedSettledCompartment'
...
$countAccessibleSeatsForVehicleBody = Get-CMethodBody $watchdog '^\s*int\s+CountAccessibleSeatsForVehicle\s*\(' 'CountAccessibleSeatsForVehicle'
$inspectVehicleCapacityBody = Get-CMethodBody $watchdog '^\s*bool\s+InspectVehicleCapacity\s*\(' 'InspectVehicleCapacity'
...
Assert-TextSequence $startBoardingPhaseBody @('phase == AICF_EVehicleBoardingPhase\.APPROACH', 'DetachVehicleFromGroup\(runtime\)', 'StartBoardingApproachActions\(runtime, slot\.GetGroup\(\), runtime\.GetVehicle\(\)\)', 'phase == AICF_EVehicleBoardingPhase\.DRIVER', 'CreateCrewRecoveryAction\(runtime, driverAgent, EAICompartmentType\.Pilot\)', 'TrackCrewRecovery\(driverAgent, driverAction\)', 'phase == AICF_EVehicleBoardingPhase\.GUNNER', 'CreateCrewRecoveryAction\(runtime, gunnerAgent, EAICompartmentType\.Turret\)', 'TrackCrewRecovery\(gunnerAgent, gunnerAction\)', 'phase == AICF_EVehicleBoardingPhase\.PASSENGERS', 'CancelBoardingApproachActions\(runtime\)', 'm_PassengerBoarding\.Start\(runtime, slot\.GetGroup\(\), issuedPassengerActions\)', 'EXACT_PER_MEMBER_CARGO', 'SetBoardingPhase\(phase\)', 'RestartPhaseDeadline\(\)') 'Exact per-member staging and mandatory crew actions must precede the exact cargo phase and its fresh bounded deadline'
Assert-TextNotContains $startBoardingPhaseBody 'CreatePassengerBoardingWaypoint|AttachVehicleToGroup\(|AddUsableVehicle\(' 'Initial boarding must never expose a generic cargo waypoint or group vehicle utility'
Assert-FileContains $runtime 'class\s+AICF_VehiclePassengerActionToken[\s\S]*SCR_AIGetInVehicle\s+m_Action[\s\S]*BaseCompartmentSlot\s+m_Compartment[\s\S]*IEntity\s+m_ReservedEntity[\s\S]*GetCompartment\(\)[\s\S]*GetReservedEntity\(\)' 'Runtime passenger tokens must retain the exact action, compartment, and reservation owner'
Assert-TextSequence $trackPassengerActionBody @('!agent \|\| !action \|\| !compartment \|\| !reservedEntity', 'm_aPassengerBoardingActions\.Insert\(new AICF_VehiclePassengerActionToken', 'agent', 'action', 'compartment', 'reservedEntity', 'retryCount') 'Runtime tracking must atomically retain every exact passenger action and reservation identity'
Assert-TextSequence $passengerStartBody @('CancelAll\(runtime\)', 'array<AIAgent> pendingAgents', 'array<IEntity> pendingEntities', 'array<SCR_AIUtilityComponent> pendingUtilities', 'array<BaseCompartmentSlot> assignedCompartments', 'group\.GetAgents\(agents\)', 'FindAvailableCargoCompartment\(', 'assignedCompartments\.Insert\(compartment\)', 'pendingAgents\.IsEmpty\(\)', 'for \(int reserveIndex', 'SetReserved\(reservedEntity\)', '!reservedCompartment\.IsReservedBy\(reservedEntity\)', 'RollbackReservations\(assignedCompartments, pendingEntities\)', 'return false;', 'for \(int actionIndex', 'new SCR_AIGetInVehicle\(', 'assignedCompartments\[actionIndex\]', 'EAICompartmentType\.Cargo', 'AddAction\(action\)', 'TrackPassengerBoardingAction\(', 'return issuedCount == pendingAgents\.Count\(\)') 'Passenger start must build and reserve the complete Cargo plan before adding its first per-member action'
Assert-TextSequence $passengerStartBody @('GetVehicleIn\(entity\) == runtime\.GetVehicle\(\)', '!IsSupportedSettledCompartment\(runtime, entity\)', 'return false;', 'continue;') 'Passenger start must reject an already-linked member unless it is settled in a role-compatible exact compartment'
Assert-TextSequence $passengerFindCargoBody @('BaseCompartmentManagerComponent', 'GetCompartments\(compartments\)', 'CargoCompartmentSlot\.Cast\(compartment\)', '!cargo', 'excluded\.Contains\(compartment\)', '!compartment\.IsCompartmentAccessible\(\)', 'compartment\.GetOccupant\(\)', 'compartment\.IsReserved\(\)', 'compartment\.IsGetInLockedFor\(entity\)', 'selected = compartment', 'return true') 'Passenger seat selection must accept only an exact accessible, empty, unreserved Cargo compartment'
Assert-TextSequence $passengerMaintainBody @('IsMemberSettledInVehicle\(tokenEntity, vehicle\)', 'access\.GetCompartment\(\) != token\.GetCompartment\(\)', 'WRONG_COMPARTMENT', 'ReleaseTracking\(runtime, token\)', 'GetVehicleIn\(tokenEntity\) == vehicle', 'continue;', 'access && \(access\.IsGettingIn\(\) \|\| access\.IsGettingOut\(\)\)', 'continue;', 'GetActionState\(\)', 'retryCount >= MAX_RETRIES', 'Cancel\(runtime, token\)', 'IssueOne\(runtime, tokenAgent, retryCount \+ 1\)', 'transition_fenced=1') 'Passenger polling must verify the reserved exact compartment and fence an in-flight transition before retry or cancellation'
Assert-TextSequence $passengerMaintainBody @('array<AIAgent> agents', 'group\.GetAgents\(agents\)', 'trackedToken = runtime\.FindPassengerBoardingAction\(agent\)', 'ResolveAccess\(entity\)', 'trackedToken \|\| \(access && \(access\.IsGettingIn\(\) \|\| access\.IsGettingOut\(\)\)\)', 'continue;', 'GetVehicleIn\(entity\) == vehicle', 'IssueOne\(runtime, agent, 0\)') 'Passenger repair must let tracked or transitioning exact actions finish before linked-seat validation or a replacement action'
Assert-TextSequence $passengerMaintainBody @('trackedToken = runtime\.FindPassengerBoardingAction\(agent\)', 'trackedToken \|\| \(access && \(access\.IsGettingIn\(\) \|\| access\.IsGettingOut\(\)\)\)', 'continue;', 'GetVehicleIn\(entity\) == vehicle', '!IsSupportedSettledCompartment\(runtime, entity\)', 'UNSUPPORTED_COMPARTMENT', 'return false;', 'continue;', 'IssueOne\(runtime, agent, 0\)') 'Passenger maintenance must validate only untracked pre-mounted members as settled and role-compatible before considering a new action'
Assert-TextSequence $passengerSupportedSettledBody @('IsMemberSettledInVehicle\(entity, runtime\.GetVehicle\(\)\)', 'ResolveAccess\(entity\)', 'access\.GetCompartment\(\)', 'PilotCompartmentSlot\.Cast\(compartment\)', 'CargoCompartmentSlot\.Cast\(compartment\)', 'return true;', 'runtime\.GetKind\(\) == AICF_EVehicleKind\.ARMED_LIGHT', 'TurretCompartmentSlot\.Cast\(compartment\)') 'Already-linked members must be physically settled in pilot, Cargo, or armed-only turret compartments'
Assert-TextSequence $countAccessibleSeatsForVehicleBody @('GetCompartments\(compartments\)', '!compartment\.IsCompartmentAccessible\(\)', 'compartment\.GetOccupant\(\)', 'compartment\.IsReserved\(\)', 'PilotCompartmentSlot\.Cast\(compartment\)', 'hasPilot = true', 'supportedSeat = true', 'CargoCompartmentSlot\.Cast\(compartment\)', 'supportedSeat = true', 'TurretCompartmentSlot\.Cast\(compartment\)', 'hasTurret = true', 'supportedSeat = kind == AICF_EVehicleKind\.ARMED_LIGHT', 'if \(supportedSeat\)', 'count\+\+') 'Capacity preflight must count only role-compatible pilot, Cargo, and armed-only turret compartments'
Assert-TextSequence $inspectVehicleCapacityBody @('CountAccessibleSeatsForVehicle\(vehicle, kind, hasPilot, hasTurret\)', 'availableSeats < requiredSeats', '!hasPilot', 'return false;', 'kind != AICF_EVehicleKind\.ARMED_LIGHT', 'hasTurret && requiredSeats >= 2') 'Candidate capacity acceptance must apply the exact supported-seat count and mandatory crew roles'
Assert-TextSequence $passengerCancelBody @('GetActionState\(\)', 'state != EAIActionState\.COMPLETED', 'state != EAIActionState\.FAILED', '!linked', 'IsAliveCharacter\(currentEntity\)', 'utility && utility\.m_OwnerEntity == currentEntity', 'action\.Fail\(\)', 'ReleaseReservation\(token\)', 'RemovePassengerBoardingAction\(token\)') 'Passenger cancellation must fail only a live owned action, then release its reservation and token'
Assert-TextSequence $passengerReleaseTrackingBody @('ReleaseReservation\(token\)', 'RemovePassengerBoardingAction\(token\)') 'Settled passenger tracking must release the exact reservation before removing its token'
Assert-TextSequence $passengerRollbackBody @('compartments\.Count\(\)', 'entities\.Count\(\)', 'IsReservedBy\(entities\[index\]\)', 'SetReserved\(null\)') 'A partial atomic-reservation failure must roll back only reservations owned by the pending members'
Assert-TextSequence $passengerReleaseReservationBody @('token\.GetCompartment\(\)', 'token\.GetReservedEntity\(\)', 'IsReservedBy\(token\.GetReservedEntity\(\)\)', 'SetReserved\(null\)') 'Passenger cleanup must never release another action owner reservation'
Assert-TextSequence $passengerCancelAllBody @('GetPassengerBoardingActionCount\(\)', 'for \(int index', 'Cancel\(runtime, runtime\.GetPassengerBoardingAction\(index\)\)', 'return count') 'Fallback cleanup must cancel every exact passenger token through the owner-safe path'
Assert-TextSequence $passengerClearTrackingBody @('GetPassengerBoardingActionCount\(\)', 'ReleaseTracking\(runtime, runtime\.GetPassengerBoardingAction\(index\)\)', 'ClearPassengerBoardingActions\(\)') 'Successful completion must release every exact passenger reservation before clearing runtime tracking'
...
Assert-TextSequence $processBoardingBody @('MarkBoardingOwnershipReportDue\(BOARDING_ACTION_OWNERSHIP_INTERVAL_MS\)', 'BOARDING_ACTION_OWNERSHIP', 'DescribeCrewBoardingAction\(runtime\)', 'memberSamples', 'm_PassengerBoarding\.Describe\(runtime\)', 'BOARDING_CREW_ROLE_LOST', 'CREW_ROLE_LOST_DURING_BOARDING', 'LatchAcceptanceFailure', 'BeginFallback') 'Boarding must capture exact crew and passenger action/seat ownership periodically and at the exact crew-role loss edge'
Assert-TextSequence $passengerDescribeBody @('GetPassengerBoardingActionCount', 'GetCompartmentMgrID', 'GetCompartmentSlotID', 'IsReservedBy', 'access\.GetCompartment', 'assigned_%5/%6', 'reserved_%7', 'actual_%8/%9') 'Passenger ownership diagnostics must expose exact assigned manager/slot, reservation ownership, and actual compartment'
...
$passengerPhaseBody = Get-CGuardBody $processBoardingBody '(?m)^[ \t]*if \(phase == AICF_EVehicleBoardingPhase\.PASSENGERS\)[ \t]*(?=\r?$)' 'ProcessBoarding PASSENGERS phase'
...
$allPassengersMountedBody = Get-CGuardBody $passengerPhaseBody '(?m)^[ \t]*if \(settledPolls >= BOARDING_SETTLED_POLLS_REQUIRED\)[ \t]*(?=\r?$)' 'ProcessBoarding stable settled PASSENGERS branch'
...
Assert-TextSequence $passengerPhaseBody @('!driverSettled', '!gunnerSettled', 'CREW_ROLE_LOST_DURING_BOARDING', 'BeginFallback', 'return;', 'm_PassengerBoarding\.Maintain\(runtime, group, passengerFailureReason\)', 'PASSENGER_BOARDING_ACTION_FAILED', 'settledCount == aliveCount', 'AreAllAliveMembersSettledInVehicle', 'RecordBoardingSettledPoll', 'BOARDING_SETTLED_POLLS_REQUIRED') 'PASSENGERS phase must reject lost mandatory crew before maintaining cargo actions, then require two fully settled polls'
...
Assert-TextSequence $completeBoardingBody @('GetBoardingSettledPollCount\(\) < BOARDING_SETTLED_POLLS_REQUIRED', 'AreAllAliveMembersSettledInVehicle', 'return;', 'CountAliveGroupMembersInVehicle', 'AttachVehicleToGroup\(runtime, slot\)', 'DeleteRuntimeWaypoint', 'ClearCrewRecoveryTracking', 'm_PassengerBoarding\.ClearTracking\(runtime\)', 'SetBoardingPhase\(AICF_EVehicleBoardingPhase\.NONE\)', '"BOARDING_COMPLETE"', 'StartMovement\(') 'Shared boarding completion must revalidate full settlement, attach utility only after crew, clean exact cargo tracking, and clear the phase before movement'
...
$guideDismountedBody = Get-CMethodBody $coordinator '^\s*protected\s+int\s+GuideTrappedDismountedMembers\s*\(' 'GuideTrappedDismountedMembers'
...
$recoverDismountClearanceBody = Get-CMethodBody $coordinator '^\s*protected\s+bool\s+RecoverProtectedDismountClearance\s*\(' 'RecoverProtectedDismountClearance'
...
$dismountReissueGuardBody = Get-CGuardBody $processDismountBody 'if \(!runtime\.IsDismountReissueAttempted\(\) && dismountAgeMs >= dismountTimeoutMs / 2 &&\s*\(logicalOccupants > 0 \|\| transitions > 0\)\)' 'ProcessDismount bounded reissue branch'
...
Assert-TextSequence $processDismountBody @('InspectProtectedMemberDismountClearance', 'ObserveDismountClearance', 'clearPolls >= DISMOUNT_CLEAR_POLLS_REQUIRED', 'CompleteDismount', 'physicalOnlyBlocked', 'GetDismountClearanceBlockedAgeMs', 'CanAttemptDismountClearanceRecovery', 'GuideTrappedDismountedMembers', 'RecordDismountClearanceRecoveryAttempt', 'DISEMBARK_CLEARANCE_GUIDANCE', 'dismountAgeMs >= dismountTimeoutMs', '"DISEMBARK_TIMEOUT"', 'LatchAcceptanceFailure', 'BeginFallback', '!runtime\.IsDismountReissueAttempted\(\)', 'dismountTimeoutMs / 2', 'MarkDismountReissueAttempted', 'ResetGroupVehicleActions', 'IssueDismountWaypoint', '"DISEMBARK_REISSUED"') 'Normal dismount must prove stable clearance, issue bounded per-member movement guidance before timeout, then enforce its hard timeout and one reissue'
Assert-TextNotContains $processDismountBody 'RelocateTrappedDismountedMembers|\.Teleport\(|SetOrigin\(|SetWorldTransform\(' 'Normal dismount polling must never teleport or relocate a trapped member'
...
Assert-TextSequence $guideDismountedBody @('Replication\.IsServer\(\)', 'group\.GetAgents\(agents\)', 'IsAliveCharacter\(character\)', 'FindBoardingApproachAction\(agent\)', 'GetVehicleIn\(character\) == vehicle', 'IsInCompartment\(\)', 'IsGettingIn\(\)', 'IsGettingOut\(\)', 'IsInsideExpandedDismountBounds', 'CancelBoardingApproachAction\(runtime, existing\)', 'existingState != EAIActionState\.COMPLETED', 'existingState != EAIActionState\.FAILED', 'alreadyActive\+\+', 'searchAttempts\+\+', 'FindEmptyTerrainPosition', 'IsInsideExpandedDismountBounds', 'TryGetWaterSurfaceSimple', 'utility\.m_OwnerEntity != character', 'new SCR_AIMoveIndividuallyBehavior', 'AddAction\(action\)', 'TrackBoardingApproachAction', 'newlyIssued\+\+', 'return newlyIssued') 'Normal clearance guidance must distinguish active actions from fresh search attempts and target each logically-out trapped member with an owned movement action to verified dry empty terrain'
Assert-TextSequence $processDismountBody @('GuideTrappedDismountedMembers', 'if \(searchAttempts > 0\)', 'RecordDismountClearanceRecoveryAttempt') 'An already-running per-member guidance action must not consume the bounded dismount-recovery attempt budget'
Assert-TextNotContains $guideDismountedBody '\.Teleport\(|SetOrigin\(|SetWorldTransform\(|GetOutVehicle_NoDoor' 'Normal bounded clearance guidance must be non-teleporting and non-terminal'
Assert-TextSequence $recoverDismountClearanceBody @('physicalOnlyBlocked', '!allowRelocation', 'GetDismountClearanceBlockedAgeMs', 'CanAttemptDismountClearanceRecovery', 'return safelyClear;', 'RelocateTrappedDismountedMembers', 'RecordDismountClearanceRecoveryAttempt') 'Destructive relocation must remain behind the explicit terminal/fallback recovery gate and bounded attempt budget'
Assert-TextSequence $relocateDismountedBody @('Replication\.IsServer\(\)', 'GetVehicleIn\(character\) == vehicle', 'IsInCompartment\(\)', 'IsGettingIn\(\)', 'IsGettingOut\(\)', 'IsInsideExpandedDismountBounds', 'FindEmptyTerrainPosition', 'character\.Teleport') 'Terminal/fallback clearance recovery may relocate only a logically-out alive member from inside vehicle bounds to verified empty terrain'
Assert-OccurrenceCount $coordinatorText 'RelocateTrappedDismountedMembers\(' 2 'Only the relocation helper and terminal/fallback recovery gate may reference teleport clearance recovery'
...
Assert-TextSequence $forceOutBody @('IsAliveCharacter', 'CompartmentAccessComponent\.GetVehicleIn', 'InterruptVehicleActionQueue', 'forceAttempt', 'exactEscalation', '!exactEscalation', 'FindSuitableTeleportLocation', 'GetOutVehicle_NoDoor', 'access\.GetCompartment\(\)', 'compartment\.GetVehicle\(\) == vehicle', 'compartment\.GetOccupant\(\) == character', 'exactEjectNeeded', 'EjectOccupant\(true, false, ejectedImmediately, false\)', '"FORCE_DISEMBARK_MEMBER"') 'Forced fallback must operate only on alive same-vehicle members, try the normal no-door escape once, then escalate to owner-directed exact-compartment ejection with per-member evidence'
Assert-TextSequence $processTerminalBody @('EmitAbandonedExitAudit', 'runtime\.IsInfantryFallbackRestorePending\(\)', 'RecoverProtectedDismountClearance', 'BeginForceDismountAttempt', 'ForceAliveGroupMembersOut', 'if \(groupCurrent\)', 'InspectProtectedMemberDismountClearance', 'RestoreInfantryOrder', 'if \(restored\)', 'ClearInfantryFallbackRestorePending', 'if \(!groupOut\)', 'return;', '!RecoverProtectedDismountClearance', 'IsFunctionalAbandonedVehicle', 'CanDeleteVehicleSafely') 'Terminal polling must audit pending clearance, issue bounded exact-member exits, restore the living group order before vehicle clearance completes, and release or delete only after global safe-clear'
...
Assert-TextSequence $processWaitingBody @('(?:prefab\.IsEmpty\(\)|prefabCandidates\.IsEmpty\(\))', 'RecordVehicleTerminalFailure', 'LatchAcceptanceFailure', 'BeginFallback', 'return;') 'A missing configured prefab must terminate rather than wait forever'
...
Assert-FileContains $coordinator 'case AICF_EVehicleState\.ABANDONED:[\s\S]*case AICF_EVehicleState\.DESTROYED:[\s\S]*Terminal cleanup[\s\S]*return false;' 'ABANDONED/DESTROYED cleanup must never retain movement ownership or suppress the living group strategic order'
...
Assert-FileContains $coordinator 'ReplanControlledMovement[\s\S]*(?:AssignOrder|ReconcileStrategicOrder)[\s\S]*SuspendOrderForVehicle' 'A graph rebuild must update the strategic target without leaving an infantry waypoint beside the vehicle waypoint'
...
$requiredEvents = @(
    ...
    'PASSENGER_BOARDING_ACTION_FAILED', 'BOARDING_TRANSITION_GRACE',
    'BOARDING_PROGRESS', 'BOARDING_COMPLETE', 'BOARDING_TIMEOUT', 'VEHICLE_ROUTE_ASSIGNED', 'VEHICLE_PROGRESS', 'VEHICLE_MOTION', 'DISEMBARK_STARTED',
    'DISEMBARK_CLEARANCE_GUIDANCE', 'DISEMBARK_CLEARANCE_RECOVERY', 'DISEMBARK_REISSUED', 'DISEMBARK_TIMEOUT', 'DISEMBARK_COMPLETE', 'VEHICLE_STUCK_DETECTED', 'VEHICLE_RECOVERY_STARTED',
    ...
    'DRIVER_REASSIGNED', 'GUNNER_LOST', 'GUNNER_REASSIGNED', 'FALLBACK_FORCE_DISEMBARK', 'FALLBACK_DISEMBARK_FAILED',
