...
$createApproachWaypointBody = Get-CMethodBody $waypoints '^\s*AIWaypoint\s+CreateBoardingApproachWaypoint\s*\(' 'CreateBoardingApproachWaypoint'
...
Assert-TextContains $coordinatorText 'BOARDING_APPROACH_ACTIVATION_WINDOW_MS\s*=\s*5000\s*;' 'A queued approach waypoint must receive a finite five-second activation window'
...
Assert-TextSequence $startBoardingPhaseBody @('phase == AICF_EVehicleBoardingPhase\.APPROACH', 'DetachVehicleFromGroup\(runtime\)', 'CreateBoardingApproachWaypoint', 'AddWaypointAt', 'SetActiveWaypoint', 'phase == AICF_EVehicleBoardingPhase\.DRIVER', 'CreateCrewRecoveryAction\(runtime, driverAgent, EAICompartmentType\.Pilot\)', 'TrackCrewRecovery\(driverAgent, driverAction\)', 'phase == AICF_EVehicleBoardingPhase\.GUNNER', 'CreateCrewRecoveryAction\(runtime, gunnerAgent, EAICompartmentType\.Turret\)', 'TrackCrewRecovery\(gunnerAgent, gunnerAction\)', 'phase == AICF_EVehicleBoardingPhase\.PASSENGERS', 'AttachVehicleToGroup\(runtime, slot\)', 'CreatePassengerBoardingWaypoint') 'MOVE-only staging and exact mandatory crew must precede cargo utility attachment'
Assert-TextSequence $createApproachWaypointBody @('MOVE_WAYPOINT_PREFAB', 'vehicle\.GetOrigin\(\)', 'SetCompletionRadius') 'The staging waypoint must be a plain infantry Move at the vehicle, never a GetIn waypoint'
Assert-TextNotContains $createApproachWaypointBody 'SetAllowance|SetEntity|GET_IN_WAYPOINT_PREFAB' 'Staging must not expose any compartment allowance'
...
Assert-TextSequence $processBoardingBody @('InspectBoardingProgress', 'ObserveBoardingProgress', 'DescribeBoardingWaypointState', 'AICF_EVehicleBoardingPhase\.APPROACH', 'farthestDistanceMeters <= stagingThresholdMeters', 'BOARDING_APPROACH_COMPLETE', 'ContinueRoleOrderedBoarding', 'ResolveAliveDriver\(runtime\)', 'IsAliveGroupMember\(group, driver\)', 'ResolveAliveGunner\(runtime\)', 'IsAliveGroupMember\(group, gunner\)') 'Boarding must stage every alive member, expose physical progress, then validate current-group crew'
Assert-TextSequence $processBoardingBody @('approachInQueue = approachWaypoint && approachQueue\.Contains', 'approachCurrent = approachWaypoint && group\.GetCurrentWaypoint\(\) == approachWaypoint', 'approachActivationExpired', '!approachWaypoint \|\| !approachInQueue \|\| \(approachActivationExpired && !approachCurrent\)', 'BOARDING_APPROACH_LOST', 'LatchAcceptanceFailure\(runtime, "BOARDING_APPROACH_WAYPOINT_LOST"\)', 'BeginFallback') 'Approach activation may be eventually consistent only inside its bounded window; a lost candidate must invalidate acceptance before fallback'
...
$protectedTerminalBody = Get-CGuardBody $processTerminalBody 'if \(runtime\.GetVehicle\(\) && m_Watchdog\.HasProtectedOccupant\(runtime\.GetVehicle\(\)\)\)' 'ProcessTerminal protected occupant guard'
...
Assert-TextSequence $processTerminalBody @('runtime\.IsInfantryFallbackRestorePending\(\)', 'RecoverProtectedDismountClearance', 'ForceAliveGroupMembersOut', 'if \(groupOut && groupCurrent\)', 'RestoreInfantryOrder', 'ClearInfantryFallbackRestorePending', 'logicalOccupants == 0 && transitions == 0 && insideBounds > 0', 'RestoreInfantryOrder\(runtime, faction, slot, "PHYSICAL_CLEARANCE_PENDING"\)', 'ClearInfantryFallbackRestorePending', 'if \(!groupOut\)', 'return;', '!RecoverProtectedDismountClearance', 'GetCleanupAtMs', 'HasProtectedOccupant') 'Terminal polling may restore movement for a logically-out physical blocker, but must retain the vehicle and block deletion until verified safe clearance'
Assert-TextSequence $processTerminalBody @('runtime\.IsInfantryFallbackRestorePending\(\) && !runtime\.GetGroup\(\)', 'ClearInfantryFallbackRestorePending', '"INFANTRY_FALLBACK_RESTORE_SKIPPED"', 'runtime\.IsInfantryFallbackRestorePending\(\)', 'HasProtectedOccupant') 'A vanished stale group must release only its impossible restore obligation while retaining global occupant protection'
Assert-TextSequence $processTerminalBody @('slot\.GetSpawnGeneration\(\) > runtime\.GetGroupGeneration\(\)', 'TryGetDesiredKind', 'CanStartVehicleTrip', 'cleanupDue = System\.GetTickCount\(\) >= runtime\.GetCleanupAtMs\(\)', '!cleanupDue && !expediteForReplacement', 'HasProtectedOccupant', 'RequestVehicleDelete', 'FinalizeVehicleCleanup') 'An empty stale terminal vehicle may request early deletion only after protected occupant and clearance guards'
Assert-TextSequence $protectedTerminalBody @('ForceAliveGroupMembersOut', 'if \(m_Watchdog\.HasProtectedOccupant\(runtime\.GetVehicle\(\)\)\)', 'return;') 'Replacement acceleration must still stop cleanup while any protected occupant remains'
...
Assert-TextSequence $processVehicleDeleteBody @('FindEntityByID\(runtime\.GetVehicleDeleteEntityId\(\)\)', 'CanRetryVehicleDelete', 'RecordVehicleDeleteRetry', '"VEHICLE_DELETE_RETRIED"', 'RplComponent\.DeleteRplEntity\(remaining, false\)', 'GetVehicleDeleteAgeMs', 'VEHICLE_DELETE_CONFIRM_TIMEOUT_MS', 'LatchAcceptanceFailure', '"VEHICLE_DELETE_NOT_CONFIRMED"', 'FinalizeVehicleCleanup') 'Authority confirmation must retry bounded deletion, fail closed on timeout, and finalize only when the entity no longer resolves'
...
$requiredEvents = @(
    ...
    'VEHICLE_DESTROYED', 'VEHICLE_CAP_BLOCKED', 'VEHICLE_DELETE_REQUESTED',
...
Write-Host 'Checked: authority, disabled default, faction assignment, distance-gated and seat-normalized role boarding, one-shot role repair, immutable boarding/dismount deadlines, cumulative acceptance failures, non-final READY result candidate, exact/deduplicated spawn failures, terminal retry latch, road-reachable routing, route/physical progress, bounded mobility recovery, protected cleanup, generations, waypoint/entity cleanup, infantry fallback, always-global group markers, callback absence, diagnostics contract.'
