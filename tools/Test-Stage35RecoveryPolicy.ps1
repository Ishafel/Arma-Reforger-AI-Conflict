...
Assert-Contains $stage3 'DEFAULT_BOARDING_TIMEOUT_MS = 25000' 'TIMING_DEFAULTS'
Assert-Contains $stage3 'DEFAULT_BOARDING_APPROACH_TIMEOUT_MS = 60000' 'TIMING_DEFAULTS'
Assert-Contains $stage3 'DEFAULT_PASSENGER_BOARDING_TIMEOUT_MS = 30000' 'TIMING_DEFAULTS'
Assert-Contains $stage3 'DEFAULT_DISMOUNT_TIMEOUT_MS = 20000' 'TIMING_DEFAULTS'
...
Assert-Contains $tokens 'class AICF_VehiclePassengerSeatPlanEntry' 'PARTIAL_EXACT_CARGO_PLAN'
Assert-Contains $tokens 'class AICF_VehiclePassengerSeatPlan' 'PARTIAL_EXACT_CARGO_PLAN'
Assert-Contains $boarding 'IssueReadyPassengerPlanActions(' 'PARTIAL_EXACT_CARGO_PLAN'
Assert-Contains $boarding 'blockedCount++' 'PARTIAL_EXACT_CARGO_PLAN'
Assert-Contains $boarding 'DETERMINISTIC_PARTIAL_EXACT_CARGO_AFTER_MANDATORY_CREW' 'PARTIAL_EXACT_CARGO_PLAN'
Assert-Contains $boarding 'transient occupancy, accessibility or reservation state' 'PARTIAL_EXACT_CARGO_PLAN'
Assert-Contains $tokens 'ReleaseReservationOwnerSafe()' 'PARTIAL_EXACT_CARGO_RESERVATION_OWNER'
Assert-Contains $boarding 'DescribePassengerSeatPlan(' 'POST_CREW_CARGO_ALLOCATION_DIAGNOSTICS'
Assert-Contains $boarding 'reserved_by_member=%5' 'POST_CREW_CARGO_ALLOCATION_DIAGNOSTICS'
...
Assert-Contains $boarding 'PASSENGER_HIDDEN_EXACT_CARGO_DEFERRED' 'EXACT_CARGO_RECOVERY_WAIT'
Assert-Contains $boarding 'action=WAIT_UNTIL_PHASE_DEADLINE' 'EXACT_CARGO_RECOVERY_WAIT'
...
Assert-Contains $boarding 'EXACT_CARGO_READY_RETRY_STALL_MS = 5000' 'NORMAL_FIRST_EXACT_CARGO'
Assert-Contains $boarding 'EXACT_SEAT_RECOVERY_OBSERVATION_MS = 5000' 'NORMAL_FIRST_EXACT_CARGO'
Assert-Contains $boarding 'PASSENGER_ANIMATED_EXACT_CARGO_REQUESTED' 'NORMAL_FIRST_EXACT_CARGO'
Assert-Contains $boarding '(transitioning && !animatedRecoveryExpired)' 'EXACT_CARGO_TRANSITION_RACE'
Assert-Contains $tokens 'ReissueTrackedExactSeatAction(' 'TRACKED_EXACT_SEAT_REISSUE'
...
Assert-Contains $tokens 'SCR_AIGetInVehicle previousAction = m_GetInAction;' 'TRACKED_EXACT_SEAT_REISSUE'
Assert-Contains $tokens 'previousAction.Fail();' 'TRACKED_EXACT_SEAT_REISSUE'
Assert-Contains $tokens 'SCR_AIGetInVehicle replacementAction = new SCR_AIGetInVehicle(' 'TRACKED_EXACT_SEAT_REISSUE'
Assert-Contains $tokens 'utility.AddAction(replacementAction);' 'TRACKED_EXACT_SEAT_REISSUE'
Assert-Contains $boarding 'direct_component_request=0' 'TRACKED_EXACT_SEAT_NO_DIRECT_COMPONENT_REQUEST'
$trackedExactReissue = [regex]::Match(
    $tokens,
    '(?s)bool ReissueTrackedExactSeatAction\(.*?(?=\r?\n\tbool WasAnimatedExactSeatRecoveryAttempted\()'
)
if (-not $trackedExactReissue.Success) {
    $issues.Add('[TRACKED_EXACT_SEAT_REISSUE] method body not found')
} else {
    Assert-NotContains $trackedExactReissue.Value 'access.GetInVehicle(' 'TRACKED_EXACT_SEAT_NO_DIRECT_COMPONENT_REQUEST'
}
Assert-Ordered $boarding 'PASSENGER_ANIMATED_EXACT_CARGO_REQUESTED' 'PASSENGER_HIDDEN_EXACT_CARGO_SCHEDULED' 'NORMAL_BEFORE_HIDDEN_EXACT_CARGO'
Assert-Contains $tokens 'EAICompartmentType.Pilot' 'EXACT_CREW_RECOVERY'
Assert-Contains $tokens 'EAICompartmentType.Turret' 'EXACT_CREW_RECOVERY'
Assert-Contains $boarding 'CREW_HIDDEN_EXACT_ROLE_FORCED' 'EXACT_CREW_RECOVERY'
Assert-Contains $boarding 'token.ObserveSpatialProgress(distanceMeters, APPROACH_PROGRESS_METERS)' 'LIVE_CREW_PROGRESS'
Assert-Contains $boarding 'IsHiddenRecoveryCombatSafe(group, threatMeasure)' 'BOARDING_COMBAT_FENCE'
Assert-Contains $boarding 'HIDDEN_RECOVERY_MAX_VEHICLE_SPEED_MPS' 'BOARDING_STATIONARY_FENCE'
Assert-Contains $boarding 'CREW_AGENT_SELECTED' 'DETERMINISTIC_NEAREST_CREW'
Assert-Contains $boarding 'entityId.Compare(selectedEntityId) < 0' 'DETERMINISTIC_NEAREST_CREW'
Assert-Contains $boarding 'TryRotateVisibleCrewCandidate(' 'VISIBLE_CREW_ROTATION'
Assert-Contains $boarding 'CREW_ROTATION_OBSERVATION_WINDOW_MS = 20000' 'VISIBLE_CREW_ROTATION_WINDOW'
Assert-Contains $boarding 'CREW_AGENT_ROTATION_SCHEDULED' 'TWO_TICK_CREW_ROTATION'
Assert-Contains $boarding 'WAIT_ONE_SCHEDULER_TICK_BEFORE_EXACT_ROLE_REISSUE' 'TWO_TICK_CREW_ROTATION'
Assert-Contains $boarding 'CREW_AGENT_ROTATED' 'VISIBLE_CREW_ROTATION'
Assert-Contains $boarding 'TRACKED_VISIBLE_EXACT_ROLE_REISSUE' 'VISIBLE_CREW_ROTATION'
Assert-Ordered $boarding 'CREW_AGENT_ROTATION_SCHEDULED' 'CREW_AGENT_ROTATED' 'TWO_TICK_CREW_ROTATION'
Assert-Contains $phaseStates 'ScheduleCrewRotation(' 'TWO_TICK_CREW_ROTATION'
Assert-Contains $phaseStates 'IsCrewRotationReady(' 'TWO_TICK_CREW_ROTATION'
Assert-Contains $phaseStates 'nowMs > m_iCrewRotationScheduledAtMs' 'TWO_TICK_CREW_ROTATION'
Assert-Contains $phaseStates 'RejectCrewCandidate(' 'VISIBLE_CREW_ROTATION'
Assert-Contains $phaseStates 'IsCrewCandidateRejected(' 'VISIBLE_CREW_ROTATION'
Assert-Contains $boarding 'TRACKED_EXACT_ROLE_OWNERSHIP_LOST' 'TRACKED_CREW_ORPHAN_DETECTION'
$cancelOwnerSafe = [regex]::Match(
    $tokens,
    '(?s)void CancelOwnerSafe\(\).*?(?=\r?\n\tprotected CompartmentAccessComponent ResolveAccess\()'
)
if (-not $cancelOwnerSafe.Success) {
    $issues.Add('[TERMINAL_GET_IN_CANCELLATION] method body not found')
} else {
    Assert-Contains $cancelOwnerSafe.Value 'm_GetInAction && IsActionOwnedByUtility(m_GetInAction)' 'TERMINAL_GET_IN_CANCELLATION'
    Assert-NotContains $cancelOwnerSafe.Value 'm_GetInAction && IsExactCompartmentMutationSafe()' 'TERMINAL_GET_IN_CANCELLATION_WITHOUT_RESERVATION'
}
Assert-Contains $phaseStates 'm_bGraceEvaluated = false;' 'PER_PHASE_GRACE'
Assert-Contains $phaseStates 'm_iGraceDeadlineMs = 0;' 'PER_PHASE_GRACE'
Assert-Contains $boarding 'BOARDING_FULL_OCCUPANCY_DEADLINE_SUPPRESSED' 'FULL_OCCUPANCY_NO_TIMEOUT_FALLBACK'
Assert-Contains $boarding 'BOARDING_FULL_OCCUPANCY_CONFIRMATION_PENDING' 'FULL_OCCUPANCY_NO_TIMEOUT_FALLBACK'
Assert-Contains $boarding 'action=WAIT_NEXT_TICK_NO_FALLBACK no_force_disembark=1' 'FULL_OCCUPANCY_NO_TIMEOUT_FALLBACK'
Assert-Contains $phaseStates 'MarkFullOccupancyDeadlineSuppressionReported()' 'FULL_OCCUPANCY_NO_TIMEOUT_FALLBACK'
...
Assert-Contains $spawner 'class AICF_VehicleStagingActionToken' 'SPAWN_STAGING_EXACT_MEMBER_RECOVERY'
...
Assert-Contains $acquisition 'SPAWN_BOARDING_ENVELOPE_RADIUS_METERS = 90.0' 'SPAWN_STAGING_SAFE_BOARDING_ENVELOPE'
Assert-Contains $spawner 'padDistanceMeters >= VEHICLE_CLEARANCE_RADIUS_METERS +' 'SPAWN_STAGING_SAFE_BOARDING_ENVELOPE'
Assert-Contains $spawner 'padDistanceMeters <= boardingEnvelopeRadiusMeters' 'SPAWN_STAGING_SAFE_BOARDING_ENVELOPE'
Assert-Contains $acquisition 'MaintainStagingMemberActions(' 'SPAWN_STAGING_EXACT_MEMBER_RECOVERY'
Assert-Contains $acquisition 'VEHICLE_STAGING_MEMBER_ACTION_ISSUED' 'SPAWN_STAGING_EXACT_MEMBER_RECOVERY'
Assert-Contains $acquisition 'VEHICLE_STAGING_MEMBER_ACTION_REISSUED' 'SPAWN_STAGING_BOUNDED_RETRY'
Assert-Contains $acquisition 'STAGING_MEMBER_ACTION_STALL_MS = 12000' 'SPAWN_STAGING_BOUNDED_RETRY'
Assert-Contains $acquisition 'plan.CancelStagingActionsOwnerSafe();' 'SPAWN_STAGING_TERMINAL_CANCELLATION'
Assert-Contains $acquisition 'TryRelocateOneExhaustedStagingOutlier(' 'SPAWN_STAGING_TERMINAL_RELOCATION'
Assert-Contains $acquisition 'plan.GetStagedCount() != plan.GetAliveCount() - 1' 'SPAWN_STAGING_TERMINAL_RELOCATION'
Assert-Contains $acquisition 'VEHICLE_STAGING_MEMBER_RELOCATION_ATTEMPTED' 'SPAWN_STAGING_TERMINAL_RELOCATION'
Assert-Contains $acquisition 'visibility_policy=VISIBLE_GROUND_RECOVERY player_fence=NOT_APPLICABLE' 'SPAWN_STAGING_VISIBLE_RECOVERY_POLICY'
Assert-Contains $acquisition 'outlier.Teleport(transform);' 'SPAWN_STAGING_TERMINAL_RELOCATION'
Assert-Contains $acquisition 'if (!targetChanged && !strategicIntentChanged)' 'SPAWN_PLAN_RUNTIME_REVISION_STABILITY'
Assert-Contains $acquisition 'action=KEEP_SPAWN_PLAN' 'SPAWN_PLAN_WAYPOINT_REVISION_STABILITY'
Assert-Contains $acquisition 'live_site_revalidated=1' 'SPAWN_PLAN_BASE_REVISION_STABILITY'
Assert-Contains $boarding 'STAGING_THRESHOLD_METERS = 90.0' 'BOARDING_STAGING_ENVELOPE_ALIGNMENT'
Assert-Contains $match 'RequestExplicitVehicleAdmission(' 'PLAYER_VEHICLE_REACQUISITION'
Assert-Contains $coordinator 'class AICF_ExplicitVehicleAdmissionIntent' 'PLAYER_VEHICLE_REACQUISITION'
Assert-Contains $coordinator 'VEHICLE_EXPLICIT_ADMISSION_REQUESTED' 'PLAYER_VEHICLE_REACQUISITION'
Assert-Contains $coordinator 'VEHICLE_EXPLICIT_ADMISSION_CONSUMED' 'PLAYER_VEHICLE_REACQUISITION'
Assert-Contains $coordinator 'routeMeters < m_Config.GetMinimumRouteMeters() && !explicitAdmission' 'PLAYER_VEHICLE_ROUTE_THRESHOLD_BYPASS'
Assert-Contains $coordinator 'other_gates_preserved=1' 'PLAYER_VEHICLE_ROUTE_THRESHOLD_BYPASS'
...
Assert-Contains $acquisition 'visibility_policy=VISIBLE_GROUND_RECOVERY' 'SPAWN_APPROACH_NO_HIDDEN_MUTATION'
...
Assert-Contains $dismount 'TRIP_EXIT_CLEAR_POLLS = 2' 'EARLY_TRIP_EXIT_PROOF'
Assert-Contains $phaseStates 'RecordTripExitSample(' 'EARLY_TRIP_EXIT_PROOF'
Assert-Contains $dismount 'tripExitClearPolls >= TRIP_EXIT_CLEAR_POLLS' 'EARLY_TRIP_EXIT_PROOF'
Assert-Contains $dismount 'ANIMATED_EXACT_EXIT_DELAY_MS = 5000' 'NORMAL_DISMOUNT_ESCALATION'
Assert-Contains $dismount 'FORCED_EXACT_EXIT_DELAY_MS = 10000' 'NORMAL_DISMOUNT_ESCALATION'
Assert-Contains $dismount 'TryRequestAnimatedExactManagedExits(' 'NORMAL_DISMOUNT_ESCALATION'
Assert-Contains $dismount 'TryForceExactManagedExit(trip, sample)' 'NORMAL_DISMOUNT_ESCALATION'
...
Assert-Ordered $dismount 'tripExitClearPolls >= TRIP_EXIT_CLEAR_POLLS' 'egressGuidanceRequired' 'EARLY_HANDOFF_BEFORE_STRICT_TARGET_SIDE'
