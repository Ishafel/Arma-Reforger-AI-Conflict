...
Assert-Contains 'STAGE4_ROLE_ROSTER' $groupSpawner 'EEntityCatalogType\.CHARACTER[\s\S]*SQUAD_LEADER[\s\S]*MEDIC[\s\S]*MACHINE_GUNNER[\s\S]*ANTI_TANK[\s\S]*GRENADIER[\s\S]*AUTOMATIC_RIFLEMAN[\s\S]*MACHINE_GUNNER_ASSISTANT[\s\S]*ANTI_TANK_ASSISTANT[\s\S]*RIFLEMAN' 'Managed roster must resolve a varied ten-position role table from the active faction character catalog'
...
Assert-Contains 'STAGE4_DEFERRED_VEHICLE_SPAWN' $stage3Config 'DEFAULT_SPAWN_STAGING_RADIUS_METERS\s*=\s*50\.0' 'Default vehicle staging radius must allow a dispersed living squad to assemble within fifty metres'
Assert-Contains 'STAGE4_SPAWN_STAGING_CLEARANCE' $stage3Config 'DEFAULT_SPAWN_STAGING_OFFSET_METERS\s*=\s*68\.0' 'The staging center must keep a fifty-metre rally circle outside the eight-metre spawn cylinder with margin'
...
Assert-Contains 'STAGE4_DEFERRED_VEHICLE_SPAWN' $stage3Config 'aicfVehicleSpawnStagingRadiusMeters[\s\S]*ClampFloat\s*\(\s*value\.ToFloat\s*\(\s*\)\s*,\s*5\.0\s*,\s*100\.0\s*\)' 'CLI staging-radius override must permit the fifty-metre default and bounded tuning above it'
