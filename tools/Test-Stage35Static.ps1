...
if ($match) {
    ...
    Assert-AICFContains $failures 'STAGE35_EXACT_ROSTER' $processFaction 'expectedSize\s*=\s*slot\.GetDeploymentSize\s*\([\s\S]*IsRosterSpawnRequested\s*\([\s\S]*actualCount\s*==\s*expectedSize[\s\S]*GetNumberOfMembersToSpawn\s*\([\s\S]*HasExactFactionRoster[\s\S]*MarkReady' 'Slot READY requires an issued request and the exact alive faction-correct deployment roster'
...
if ($tripController -and $transitFlow -and $dismountFlow -and $boardingFlow) {
	...
	Assert-AICFContains $failures 'STAGE35_EXACT_CARGO_STALL' $maintainPassengers 'ObserveSpatialProgress[\s\S]*GetProgressAgeMs[\s\S]*EAIActionState\.RUNNING[\s\S]*configuredStallMs[\s\S]*ReissueExactCargo' 'Every RUNNING exact Cargo token must have a bounded progress-aware exact retry before fenced hidden recovery'
